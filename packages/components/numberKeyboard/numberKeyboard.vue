<template>
	<m-overlay ref="overlay" :model-value="show" @show="overlayShow" @hide="overlayHide" :z-index="zIndex"
		:color="overlayColor" :timeout="timeout" @click.self="hide" :mount-el="mountEl" :use-padding="usePadding">
		<transition name="mvi-keyboard" @before-enter="beforeEnter" @after-enter="afterEnter"
			...
			<div ref="keyboard" class="mvi-number-keyboard" v-if="firstShow" v-show="keyboardShow" :style="boardStyle"
				...
				<div class="mvi-number-keyboard-right" v-if="showComplete || showDelete">
					...
					<div :disabled="(promiseEmpty?null:completeDisabled || null)" :class="completeBtnClass"
...
<script>
	...
	export default {
		...
		emits: ['update:modelValue', 'input', 'delete', 'complete', 'update:show', 'show', 'showing', 'shown', 'hide',
			'hidding', 'hidden'
		],
		inheritAttrs: false,
		props: {
