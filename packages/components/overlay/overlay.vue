<template>
	<teleport :disabled="!mountEl" :to="mountEl">
		<transition name="mvi-overlay-fade" @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter"
			...
			<div @click.self="closeOverlay" ref="overlay" class="mvi-overlay" v-show="overlayShow" :style="overlayStyle"
				v-bind="$attrs">
				<slot></slot>
			</div>
		</transition>
			...
			enter(el) {
				...
				if (this.$$el.offsetParent) {
					...
					if ($dap.element.getScrollHeight(this.offsetParent) > this.offsetParent.clientHeight) {
						if (this.usePadding) {
							...
							let scrollWidth = this.offsetParent.offsetWidth - this.offsetParent.clientWidth - parseFloat(
								$dap.element.getCssStyle(this.offsetParent, 'border-right-width')) - parseFloat($dap
								.element.getCssStyle(this.offsetParent, 'border-left-width'))
							//记录原先右侧内边距的值
							this.paddingRight = parseFloat($dap.element.getCssStyle(this.offsetParent, 'padding-right'))
							//设置右侧内边距值
							this.offsetParent.style.setProperty('padding-right', this.paddingRight + scrollWidth + 'px',
								'important')
						}
						//记录滚动条距离
					...
					if ($dap.element.getScrollWidth(this.offsetParent) > this.offsetParent.clientWidth) {
						if (this.usePadding) {
							//获取滚动条高度
							let scrollHeight = this.offsetParent.offsetHeight - this.offsetParent.clientHeight -
								parseFloat($dap.element.getCssStyle(this.offsetParent, 'border-bottom-width')) -
								parseFloat($dap.element.getCssStyle(this.offsetParent, 'border-top-width'))
							//记录原先底部侧内边距的值
							this.paddingBottom = parseFloat($dap.element.getCssStyle(this.offsetParent, 'padding-bottom'))
							//设置底部内边距值
							this.offsetParent.style.setProperty('padding-bottom', this.paddingBottom + scrollHeight + 'px',
								'important')
						}
						//记录滚动条距离
