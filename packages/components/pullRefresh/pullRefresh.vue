<template>
	<div class="mvi-pull-refresh" @touchstart="startPull" @touchmove="onPull" @touchend="pulled"
		...
		<div v-show="elShow" ref="refresh" class="mvi-pull-refresh-el" :style="elStyle">
			...
			<m-icon v-if="!$slots.el" :type="icon.type" :spin="icon.spin" :url="icon.url" :size="icon.size"
				:color="icon.color" />
	...
	export default {
		...
		methods: {
			...
			changeStatus() {
				...
				//modelValue值为false时状态变为下拉前状态，数据恢复初始化
				else { 
					this.amount = 0
					this.hasTouch = false
					this.$refs.refresh.style.transition = 'all 300ms'
