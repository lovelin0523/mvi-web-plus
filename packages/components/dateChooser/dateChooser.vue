<template>
	<div :class="['mvi-date-chooser',block?'mvi-date-chooser-block':'']" :data-id="`mvi-date-chooser-${uid}`">
		<div class="mvi-date-chooser-target" :data-id="`mvi-date-chooser-target-${uid}`" ref="target"
			@click="clickCalendar">
			<slot></slot>
		</div>
		<m-layer :target="`[data-id='mvi-date-chooser-target-${uid}']`" :root="`[data-id='mvi-date-chooser-${uid}']`"
				...
				<div class="mvi-date-chooser-header">
					<div class="mvi-date-chooser-header-left">
						<span @mouseenter="hoverHeader(true, 0)" @mouseleave="hoverHeader(false, 0)" @click="goPrevYear"
							:class="headerItemClass(0)" :disabled="prevYearDisabled || null">
					...
					<div class="mvi-date-chooser-header-right">
						...
						<span @mouseenter="hoverHeader(true, 6)" @mouseleave="hoverHeader(false, 6)" @click="goNextYear"
							:class="headerItemClass(6)" :disabled="nextYearDisabled || null">
							<m-icon type="angle-double-right" />
						</span>
					</div>
				</div>
				<m-calendar :view="view" :model-value="modelValue" :month-text="monthText" :week-text="weekText"
