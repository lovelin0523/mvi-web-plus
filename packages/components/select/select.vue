<template>
	<div :data-id="'mvi-select-' + uid" :class="selectClass" :disabled="disabled || null">
		<div :data-id="'mvi-select-target-' + uid" :class="targetClass" :style="targetStyle" ref="target"
			@click="trigger" :disabled="disabled || null">
		...
		<m-layer v-model="focus" :target="`[data-id='mvi-select-target-${uid}']`"
			:root="`[data-id='mvi-select-${uid}']`" :placement="placement" :offset="offset" :fixed="fixed"
			:fixed-auto="fixedAuto" :z-index="zIndex" closable :show-triangle="false" :wrapper-class="wrapperClass"
			:animation="animation" :timeout="timeout" shadow :border="false" @showing="layerShow" ref="layer">
			<div class="mvi-select-menu" ref="menu" :style="menuStyle">
				<div :class="['mvi-option','mvi-option-' + size]" @click="optionClick(item)"
					...
					v-for="(item, index) in options" :disabled="item.disabled || null">
