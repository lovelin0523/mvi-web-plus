...
class Drag {
	constructor(element, options) {
		...
		this.pageX = 0; //X坐标
		this.pageY = 0; //Y坐标
		this.guid = this._createGuid();//生成唯一值
	}
	...
	init() {
		...
		if ($util.getCssStyle(this.$el, "position") == "static" || $util.getCssStyle(this.$el, "position") ==
			'relative') {
	...
	_resize() {
		if (this.mode == "in") {
			if (this.draggableX) {
				...
				if ($util.getElementPoint(this.$el, this.$container).left >= this.$container.offsetWidth - this.$el
					.offsetWidth) {
			...
			if (this.draggableY) {
				...
				if ($util.getElementPoint(this.$el, this.$container).top >= this.$container.offsetHeight - this.$el
					.offsetHeight) {
