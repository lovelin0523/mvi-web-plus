	...
	<div :class="['mvi-table', outBorder ? 'mvi-table-border' : '']">
		<div :class="['mvi-table-header', headClass || '']" v-if="columnsData.length != 0">
			<table cellpadding="0" cellspacing="0">
				<colgroup>
					<col :ref="el=>headCols[index]=el" :style="colgroupStyle(item, index)"
						v-for="(item, index) in columnsData" />
					<col v-if="isScroll" :style="{ width: scrollWidth + 'px' }" />
				</colgroup>
				<thead>
					<tr>
						<th :ref="el=>headThs[index]=el" :class="tableHeaderThClass"
							v-for="(item, index) in columnsData">
							<div>
								<m-checkbox v-if="item.key == 'checkbox'"
									...
									@change="allSelect" :fill-color="item.fillColor || null" :label="item.value || ''">
								</m-checkbox>
		...
		<div v-else class="mvi-table-body" :style="bodyStyle" ref="body">
			<table cellpadding="0" cellspacing="0">
				<colgroup>
					<col :ref="el=>bodyCols[index]=el" :style="colgroupStyle(item, index)"
						v-for="(item, index) in columnsData" />
				</colgroup>
				<tbody>
					...
					this.$nextTick(() => {
						if (this.headThs.length > 0) {
							this.headThs.forEach((el, index) => {
								if (!this.columnsData[index].width && this.headCols.length > 0 && this
									.headCols[index] && this.bodyCols.length > 0 && this.bodyCols[index]) {
									this.headCols[index].style.width = this.bodyCols[index].style.width =
										el.offsetWidth + 'px'
								}
							})
						}
