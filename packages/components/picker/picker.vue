<template>
	<div class="mvi-picker">
		...
		<div v-if="showToolbar && !loading" class="mvi-picker-toolbar">
			<div :class="['mvi-picker-toolbar-cancel', cancelClass || '']" v-text="cancelText"
				@click="doCancel"></div>
			<div :class="['mvi-picker-toolbar-title', titleClass || '']" v-if="title" v-text="title">
			</div>
			<div :class="['mvi-picker-toolbar-confirm', confirmClass || '']" v-text="confirmText"
				@click="doConfirm"></div>
		</div>
		<div v-if="!loading" class="mvi-picker-content" :style="contentStyle" ref="content"
			@touchmove="contentTouchMove">
			<div v-for="(column, index) in computedOptions"
				:class="['mvi-picker-items', column.className || '']" :ref="el=>itemRefs[index]=el"
				...
				<div class="mvi-picker-item" v-for="(item, index2) in column.values" v-text="item"
					:style="{ height: selectHeight || '' }"></div>
			</div>
