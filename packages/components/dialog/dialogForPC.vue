<template>
	<m-modal v-model="show" :footer-padding="false" @hide="modalHide" @hidding="modalHidding" @hidden="modalHidden"
		:width="computedWidth" :z-index="computedZIndex" :radius="computedRadius" :use-padding="computedUsePadding"
		:animation="computedAnimation" @show="modalShow" @showing="modalShowing" @shown="modalShown"
		:timeout="computedTimeout" :overlay-color="computedOverlayColor" :mount-el="computedMountEl">
		...
		<template #default v-if="contentShow">
			...
			<div v-if="type == 'Prompt'" :class="['mvi-dialog-input',computedMessage?'mvi-dialog-input-mt':'']">
				<input ref="input" :type="computedInput.type=='number'?'text':computedInput.type"
					:placeholder="computedInput.placeholder" :maxlength="computedInput.maxlength" :class="inputClass"
					:style="inputStyle" v-model.trim="value" @input="inputFun" @focus="inputFocus" @blur="inputBlur" />
			...
			<div class="mvi-dialog-footer">
				<m-button v-if="type!='Alert'" :type="computedBtns.cancel.type" :color="computedBtns.cancel.color"
					:sub-color="computedBtns.cancel.subColor" :plain="computedBtns.cancel.plain"
					class="mvi-dialog-cancel" @click="cancelFun" :size="computedBtns.cancel.size">{{computedBtns.cancel.text}}</m-button>
				<m-button :type="computedBtns.ok.type" :color="computedBtns.ok.color"
					:sub-color="computedBtns.ok.subColor" :plain="computedBtns.ok.plain" class="mvi-dialog-ok"
					@click="okFun" :size="computedBtns.ok.size">{{computedBtns.ok.text}}</m-button>
			</div>
		</template>
	</m-modal>
</template>
