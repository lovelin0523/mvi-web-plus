<template>
	<div :disabled="disabled || null" :class="['mvi-input-container','mvi-input-container-'+size,border?'mvi-input-border':'',(showWordLimit && maxlength>0)?'mvi-input-container-words':'',required?'mvi-input-required':'']" :data-type="(type=='textarea'?'textarea':'input')">
		...
		<!-- 文本域 -->
		<textarea v-if="type=='textarea'" :placeholder="placeholder" :maxlength="maxlength" :disabled="disabled || null" :readonly="readonly || null"
		 :autofocus="autofocus" class="mvi-textarea" :value="inputValue" @input="input" ref="textarea" :rows="rowsFilter"
		 :name="name" :style="textareaStyle" @focus="getFocus" @blur="getBlur" autocomplete="off"></textarea>
		<!-- 输入框 -->
		<input v-else :type="inputType" :inputmode="computedInputMode" :placeholder="placeholder" :maxlength="(isDatePicker?-1:maxlength)" :disabled="disabled || null"
		 :readonly="readonly || isDatePicker || null" :autofocus="autofocus" class="mvi-input" :value="inputValue"
		...
		<!-- 日期 -->
		<m-date-native-picker v-if="isDatePicker" ref="datepicker" :type="dateType" v-model:value="computedDate"></m-date-native-picker>
	</div>
</template>
...
<script>
	...
	export default {
		...
		emits:['update:value','update:date','left-click','right-click','focus','blur','input'],
		props: {
			label: { //输入框左侧文本
				type: String,
				default: null
