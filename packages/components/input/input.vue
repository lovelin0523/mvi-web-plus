<template>
	<div :disabled="disabled || null" :class="[
		...
		<textarea v-if="type == 'textarea'" :placeholder="placeholder" :maxlength="maxlength"
			:disabled="disabled || null" :readonly="readonly || null" :autofocus="autofocus" class="mvi-textarea"
			v-model="realValue" @input="input" ref="textarea" :rows="rowsFilter" :name="name" :style="textareaStyle"
			@focus="getFocus" @blur="getBlur" autocomplete="off"></textarea>
		<!-- 输入框 -->
		<input v-else :type="inputType" :inputmode="computedInputMode" :placeholder="placeholder"
			:maxlength="isDatePicker ? -1 : maxlength" :disabled="disabled || null"
			:readonly="readonly || isDatePicker || null" :autofocus="autofocus" class="mvi-input" v-model="realValue"
			@click="callDate" @input="input" ref="input" :name="name" :style="inputStyle" @focus="getFocus"
			@blur="getBlur" autocomplete="off" />
		<!-- 清除图标 -->
