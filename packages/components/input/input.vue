<template>
	...
	>
		...
		<div @click="leftClick" class="mvi-input-left-icon" v-if="$slots.left || leftIconUrl || leftIconType">
			...
			<m-icon
				v-else-if="leftIconUrl || leftIconType"
				:type="leftIconType"
				:url="leftIconUrl"
				:spin="leftIconSpin"
				:size="leftIconSize"
		...
		<input
			...
			:placeholder="placeholder"
			:maxlength="isDatePicker ? -1 : maxlength"
			:disabled="disabled || null"
		...
		/>
		<!-- 清除图标 -->
		<div @click="doClearValue" class="mvi-input-clear-icon" v-if="clearable" v-show="showClear"><m-icon type="times-o" /></div>
		<!-- 右侧图标 -->
		<div @click="rightClick" class="mvi-input-right-icon" v-if="$slots.right || (rightIconUrl || rightIconType)">
			<slot name="right" v-if="$slots.right"></slot>
			<m-icon
				v-else-if="rightIconUrl || rightIconType"
				ref="rightIcon"
				:type="rightIconType"
				:url="rightIconUrl"
				:spin="rightIconSpin"
