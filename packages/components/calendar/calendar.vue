			...
			<div ref="list" class="mvi-calendar-date-list">
				<div v-for="(item,index) in new Array(6)" class="mvi-calendar-date-row">
					<div class="mvi-calendar-date-day" v-for="(item2,index2) in days.slice(index*7,index*7+7)">
						<div :disabled="(!item2.currentMonth) || null" :class="['mvi-calendar-date-day-item',(nonCurrentClick?active:(active&&item2.currentMonth))?'mvi-calendar-active':'',dateNowClass(item2),dateCurrentClass(item2)]" v-text="item2.date.getDate()" @click="onDateClick(item2)"></div>
					</div>
				</div>
			</div>
		</div>
